...
- (void)renderLayer{
    ...
    /*
     ...
     参数6 起始偏移量
     */
    glVertexAttribPointer(posLocation, 3, GL_FLOAT, GL_FALSE, 3*sizeof(GLfloat), (GLvoid*)0);
...
- (GLuint)initializeShader:(GLuint)shaderType filePath:(NSString*)filePath{
    ...
    if (!success) {
        ...
        NSString* errorStr = [NSString stringWithCharacters:(const unichar *)infoLog length:sizeof(infoLog)];
        NSLog(@"error str is = %@",errorStr);
        return 0;
    }
